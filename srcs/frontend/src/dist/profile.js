...
import { initiateGoogleLogin, processGoogleOAuth, setup2fa, activate2fa } from './api.js';
...
export function initProfile() {
    ...
    const twofaBtn = document.getElementById('2fa-btn');
    ...
    async function handle2FA(token, tempToken) {
        try {
    ...
    async function handle2FASetup() {
        try {
            ...
            const closeModal = () => {
                ...
                const twofaErrorMsg = document.getElementById('2fa-error');
                if (twofaErrorMsg)
                    twofaErrorMsg.textContent = '';
                const twofaTokenInput = document.getElementById('2fa-token');
                if (twofaTokenInput)
