...
import { go_section, sections, get_type_index, get_url_type, show2FAVerificationModal, update_sections } from './sections.js';
...
export async function search(friend_username) {
    try {
        ...
        if (!response.ok || !data.success) {
            ...
            if (data.isFriend)
                return new OtherUser(data.user.username, data.isFriend, data.user.isConnected, data.user.friendSince, data.user.winRate, data.user.gamesTogether, data.user.avatar);
            return new OtherUser(data.user.username, data.isFriend, data.user.isConnected, data.user.createdAt, data.user.winRate, data.user.gamesPlayed, data.user.avatar);
        }
    }
    catch (error) {
...
export async function add(friend_username) {
    try {
        if (friend_username.length > 50) {
            showError("Username too long.");
            return false;
...
export async function remove(friend_username) {
    try {
        if (friend_username.length > 50) {
            showError("Username too long.");
            return false;
...
export async function send(message, type, to = '') {
    let url;
    let body;
    try {
        if (type === 'livechat') {
...
export async function get_direct_messages(username) {
    try {
...
export async function activate2fa(token) {
    try {
...
export async function update(username, email, new_password) {
    try {
...
export async function verify2fa(token, temp_token) {
    try {
...
export async function processGoogleOAuth(code) {
    try {
...
async function completeGoogleRegistration(username, tempToken) {
    try {
...
export async function updateAvatar(file) {
    try {
        ...
        if (data.success && user) {
            const avatarElements = document.querySelectorAll('.avatar.logged-in');
...
async function check_redirect() {
    ...
    if (type_index === undefined || sections[type_index].protected === true)
