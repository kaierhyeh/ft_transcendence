...
export class AuthService {
    ...
    async validateToken(token, expectedType) {
        try {
            if (!token) {
                return { valid: false, error: 'No token provided' };
            }
            // Verify JWT signature and expiry using RSA public key
            const decoded = jwt.verify(token, CONFIG.JWT.PUBLIC_KEY, { algorithms: [CONFIG.JWT.ALGORITHM] });
    ...
    async refreshAccessToken(fastify, refreshToken, oldAccessToken) {
        try {
            ...
            const newAccessToken = jwt.sign({ userId: decoded.userId, type: 'access' }, CONFIG.JWT.PRIVATE_KEY, {
                algorithm: CONFIG.JWT.ALGORITHM,
                expiresIn: CONFIG.JWT.ACCESS_TOKEN_EXPIRY,
