...
dotenv.config();
const oauth2Client = new google.auth.OAuth2(CONFIG.OAUTH.GOOGLE_CLIENT_ID, CONFIG.OAUTH.GOOGLE_CLIENT_SECRET, CONFIG.OAUTH.GOOGLE_REDIRECT_URI);
// 📌 Route: google/token
...
export async function oauthRoutes(fastify, options) {
    fastify.post('/auth/google', async (request, reply) => {
        try {
            ...
            fastify.log.error(`Google OAuth error: ${error.message}`);
    ...
    fastify.post("/auth/google/username", async (request, reply) => {
        ...
        try {
            ...
            const emailExists = fastify.db.prepare("SELECT 1 FROM users WHERE email = ?").get(payloadData.email);
            if (emailExists)
			...
			VALUES (?, ?, ?, 1, ?)
		`).run(checked_username, payloadData.email, payloadData.avatar, payloadData.google_name);
            ...
            fastify.log.error(`Google OAuth complete-register error: ${error.message}`);
