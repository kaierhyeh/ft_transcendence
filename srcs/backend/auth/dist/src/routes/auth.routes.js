...
export async function authRoutes(fastify, options) {
    ...
    fastify.post('/auth/register', async (request, reply) => {
        try {
            ...
            const existingUser = db.prepare("SELECT id FROM users WHERE username = ? COLLATE NOCASE OR email = ?").get(checkedUsername, email);
            if (existingUser) {
    ...
    fastify.post('/auth/refresh', async (request, reply) => {
        try {
            ...
            fastify.log.error(`Token refresh error: ${error.message}`);
            return reply.code(500).send({
                success: false,
